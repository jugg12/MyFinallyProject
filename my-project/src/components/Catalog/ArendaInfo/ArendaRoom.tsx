...
import { settingsHomePage } from "../../settingsSlider/settings__Slider";
...
import qs from "qs"
import { setCityRayonHomePage } from "../../../store/slices/FilterSlice";
...
const ArendaRoom=()=>{
  ...
  useEffect(()=>{
    ...
    if (window.location.search) {
      ...
      params.rayonFilter!==undefined ?  item = [params.cityFilter,params.rayonFilter] : item = [params.cityFilter,""]
      dispatch(setCityRayonHomePage(item));
      setLoading(true);
      if(filterCityRayon.rayon!==""){
        axios.get(`/ArendaCard?city2=${filterCityRayon.city}&rayon=${filterCityRayon.rayon}`).then(({data})=>{
          setArenda(data);
          const item2 = [params.cityFilter,params.rayonFilter,data.length]
          dispatch(setCityRayonHomePage(item2));
          setLoading(false);
       })
      }
      else{
        axios.get(`/ArendaCard?city2=${filterCityRayon.city}`).then(({data})=>{
          setArenda(data);
          const item2 = [params.cityFilter,"",data.length]
          dispatch(setCityRayonHomePage(item2));
          setLoading(false);
       })
      }
      ...
      if(filterCityRayon.rayon!==""){
        axios.get(`/ArendaCard?city2=${filterCityRayon.city}&rayon=${filterCityRayon.rayon}`).then(({data})=>{
          setArenda(data);
          const item2 = [filterCityRayon.city,filterCityRayon.rayon,data.length]
          dispatch(setCityRayonHomePage(item2));
          setLoading(false);
       })
      }
        ...
        axios.get(`/ArendaCard?city2=${filterCityRayon.city}`).then(({data})=>{
          const item = [filterCityRayon.city,"",data.length];
          dispatch(setCityRayonHomePage(item));
          setArenda(data);
          setLoading(false);
       })
  ...
  return(
    Loading?
    <div className="" style={{display:"flex",width:"1378px",marginTop:"40px"}}>
  ...
  :
  <>
    <div className="conteiner" key={generateKey("slider-wrapper")} style={{height:"595px"}}>
    <Slider {...settingsHomePage} ref={sliderRef} key={generateKey("Slider")}>
      {
      Arenda.length!==0?Arenda.map((item)=>{
      return(
      <Card className="card__style" style={{width:"406px"}}>
          ...
          <div className="CardMain">
            ...
            <Card.Body className="bodyCard">
              ...
              <Card.Text className="card__text">
                ...
                <div className="btnContactsMain">
                  <div className="dropdownContacts">
                    <button className={`ContactsBtn`} id={`${item.id}`} onClick={()=>btnclick(item.id)}>
                        <div className="btnall btnallHome" >
                ...
                <Button variant="primary" className="Morebtn MorebtnHome">
                <div className="More">
