...
const validate = ({ required }) => (req, res, next) => {
    ...
    if (required && (!login || !login.trim() || !password || !password.trim())) {
      res.status(400).jsonp('login and password are required');
        return;
    }
...
const login = (req, res, next) => {
    ...
    bcrypt
        ...
        if (!same)
            throw 400;
        return new Promise((resolve, reject) => {
            jwt.sign({ login }, constants_1.JWT_SECRET_KEY, { expiresIn: constants_1.JWT_EXPIRES_IN, subject: String(user.id) }, (error, accessToken) => {
                if (error)
                    reject(error);
                else
                    resolve(accessToken);
